...
import { 
  ...
  Headphones, Search as SearchIcon, Menu, X
} from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className={cn(
        ...
        mobileOpen ? "translate-x-0 w-64 z-[70]" : "-translate-x-full lg:translate-x-0 z-50",
        ...
        <div className="p-4 flex items-center justify-between mb-2">
          <NavLink to="/" className="flex items-start justify-center focus:outline-none">
            <img
              src="/logo.png"
              alt="OrganizerHub"
              className={cn("object-contain transition-all", (collapsed && !mobileOpen) ? "w-10 h-10" : "h-9 w-auto max-w-[160px]")}
            />
          </NavLink>
                  ...
                  <div key={item.name}>
                    {item.subItems ? (
                      <>
                        ...
                        >
                          <item.icon className={cn("w-5 h-5 shrink-0", (collapsed && !mobileOpen) && "mx-auto")} />
                          {(!collapsed || mobileOpen) && <span className="font-medium text-left flex-1">{item.name}</span>}
                          {(!collapsed || mobileOpen) && (
                        ...
                        {(!collapsed || mobileOpen) && expandedMenus.includes(item.name) && (
        ...
        <div className="relative">
          ...
          >
            <div className="text-right hidden md:block">
              <p className="text-sm font-medium text-slate-800 group-hover:text-primary transition-colors">Admin</p>
            </div>
          ...
          {showDropdown && (
              <>
                ...
                <div 
                  className="absolute right-0 mt-2 w-52 bg-white py-1 rounded-md border border-slate-200 shadow-xl z-50 animate-in fade-in zoom-in-95 duration-200"
                >
                  <button 
                    type="button"
                    onClick={() => {
