...
import { 
  DollarSign, TrendingUp, Megaphone,
  History, Download, 
  Layers, CreditCard,
  Calendar
} from 'lucide-react';
...
const FinancePage = () => {
  ...
  const stats = {
    ...
    boost: formatCurrency(165.00 * mult),
    platformFees: formatCurrency(238.44 * mult),
    premiumSubs: formatCurrency(1194.00 * mult),
    standardSubs: formatCurrency(57.00 * mult),
    spotlight: formatCurrency(75.00 * mult),
    highlight: formatCurrency(90.00 * mult)
  };
  ...
  const transactionColumns = [
    ...
    { 
      ...
      accessor: (tx: any) => (
        <span className="font-medium text-slate-500 text-[11px]">{tx.email}</span>
      )
    },
    { 
      header: "Category",
      accessor: (tx: any) => (
        <span className="font-bold text-slate-700 text-[11px] max-w-[200px] truncate block">{tx.category}</span>
      )
    },
    { 
      header: "Type",
      accessor: (tx: any) => (
        <span className={cn(
          ...
          tx.type === 'Platform Fee' ? "bg-slate-100 text-slate-500" :
          tx.type === 'Refund' ? "bg-rose-50 text-rose-600" :
          tx.type.includes('Subscription') ? "bg-blue-50 text-blue-600" :
    ...
    { 
      header: "Amount",
      accessor: (tx: any) => (
         <span className={cn("font-black text-[13px] tracking-tight", tx.amount < 0 ? "text-rose-500" : "text-slate-800")}>
    ...
    { 
      ...
      accessor: (tx: any) => (
         <span className="font-black text-slate-500 text-[12px] tracking-tight">${tx.platformFee}</span>
      )
    },
    { 
      header: "Date", 
      className: "hidden lg:table-cell",
      accessor: (tx: any) => <span className="font-bold text-slate-500 text-[11px]">{tx.date}</span> 
    },
    { 
      header: "Method", 
      className: "hidden lg:table-cell",
      accessor: (tx: any) => <span className="font-bold text-slate-500 text-[11px]">{tx.method}</span> 
    },
  ];
  ...
  const filterTabs = [
    "All", "Premium Subscriptions", "Standard Subscriptions", 
    "Platform Fee", "Homepage Spotlight", "Highlighted Listing", "Refund"
  ];
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
        ...
        <StatCard title="Transactions" value={String(stats.transactions)} icon={TrendingUp} color="blue" />
        <StatCard title="Boost Revenue" value={stats.boost} icon={Megaphone} color="orange" />
        <StatCard title="Platform Fees" value={stats.platformFees} icon={Layers} color="emerald" />
        
        {/* Row 2 Stats */}
        <StatCard title="Premium Subscriptions" value={stats.premiumSubs} icon={DollarSign} color="blue" />
        <StatCard title="Standard Subscriptions" value={stats.standardSubs} icon={DollarSign} color="orange" />
        <StatCard title="Homepage Spotlight" value={stats.spotlight} icon={DollarSign} color="primary" />
        <StatCard title="Highlighted Listing" value={stats.highlight} icon={DollarSign} color="emerald" />
      </div>
          ...
          <>
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex justify-between items-center">
              <div>
                ...
                <p className={cn("font-black text-2xl tracking-tight", detailsPayload.amount < 0 ? "text-rose-500" : "text-slate-800")}>
                  {detailsPayload.amount < 0 ? `-${formatCurrency(Math.abs(detailsPayload.amount))}` : formatCurrency(detailsPayload.amount)}
                </p>
              </div>
              <div className="text-right">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Platform Fee</p>
                <p className="font-black text-lg text-slate-600 tracking-tight">${detailsPayload.platformFee}</p>
              </div>
            </div>
            <div className="space-y-2 mt-4">
              <div className="text-sm font-bold text-slate-600">
                <span className="text-slate-400 uppercase tracking-widest text-[10px] font-black">User:</span>{" "}
                <span className="text-slate-800">{detailsPayload.name} ({detailsPayload.email})</span>
              </div>
              <div className="text-sm font-bold text-slate-600">
                <span className="text-slate-400 uppercase tracking-widest text-[10px] font-black">Category:</span>{" "}
                <span className="text-slate-800">{detailsPayload.category}</span>
              </div>
              <div className="text-sm font-bold text-slate-600">
              ...
              <div className="text-sm font-bold text-slate-600 mt-2 flex items-center gap-2">
                ...
                <span className="text-slate-800 flex items-center gap-1"><CreditCard className="w-3.5 h-3.5 text-slate-400" /> {detailsPayload.method}</span>
              </div>
            </div>
          </>
        )}
