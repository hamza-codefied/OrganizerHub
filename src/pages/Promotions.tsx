...
const PromotionsPage = () => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      >
        <button className="flex items-center gap-2 primary-gradient text-white px-6 py-2.5 rounded-2xl text-xs font-black uppercase tracking-[0.2em] shadow-lg shadow-primary/20 hover:scale-105 transition-all outline-none">
          <Plus className="w-4 h-4" />
          Deploy Spotlight
        </button>
      </PageHeader>

      <div className="flex gap-2 p-1.5 bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm w-fit">
        <button onClick={() => setActiveTab('inventory')} className={cn("px-8 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all", activeTab === 'inventory' ? "bg-white shadow-xl text-primary" : "text-slate-400 hover:text-slate-600")}>Inventory Architecture</button>
        <button onClick={() => setActiveTab('requests')} className={cn("px-8 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all", activeTab === 'requests' ? "bg-white shadow-xl text-primary" : "text-slate-400 hover:text-slate-600")}>Strategic Requests</button>
      </div>
        ...
        <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
             ...
             {[
               ...
               <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                 <GlassCard className="text-center p-10 group border-white/60 shadow-2xl hover:scale-[1.02] transition-all duration-500">
                    ...
                    <button className="w-full py-4 bg-white/40 backdrop-blur-md border border-white/60 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white hover:text-primary hover:shadow-lg transition-all text-slate-500 active:scale-95">Adjust Configuration</button>
                 </GlassCard>
               </motion.div>
             ))}
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {FEATURE_REQUESTS.filter(r => r.status === 'Active').map((req, i) => (
        ...
        <div className="animate-in slide-in-from-bottom-4 duration-500">
           <DataTable 
             ...
             rowActions={[
               ...
               { label: 'Approve Placement', icon: CheckCircle2, onClick: () => {}, variant: 'success' },
               { label: 'Reject Request', icon: XCircle, onClick: () => {}, variant: 'danger' },
               { label: 'Schedule Delay', icon: Calendar, onClick: () => {} },
             ]}
           />
        </div>
      )}
