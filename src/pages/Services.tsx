...
const ServicesPage = () => {
  ...
  const [serviceForm, setServiceForm] = useState({ name: '', category: '', tags: [] as string[] });
  ...
  const handleOpenServiceModal = (service: Service | null = null) => {
    if (service) {
      ...
      setServiceForm({ name: service.name, category: service.category, tags: [...service.tags] });
    } else {
      setCurrentService(null);
      setServiceForm({ name: '', category: categoryNames[0] || '', tags: [] });
    }
    setNewTag('');
  ...
  const handleSaveService = () => {
    ...
    const tags = serviceForm.tags.filter(Boolean);
    if (currentService) {
      setServices(services.map(s =>
        s.id === currentService.id
          ? { ...s, name: serviceForm.name.trim(), category, tags }
          : s
      ));
    } else {
  ...
  const addServiceTag = () => {
    ...
    if (t && !serviceForm.tags.includes(t)) {
      setServiceForm({ ...serviceForm, tags: [...serviceForm.tags, t] });
      setNewTag('');
    }
  };

  const removeServiceTag = (tag: string) => {
    setServiceForm({ ...serviceForm, tags: serviceForm.tags.filter(x => x !== tag) });
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2 p-4 bg-slate-50 rounded-[1.5rem] border border-slate-100 min-h-[80px]">
                    {serviceForm.tags.map((tag) => (
                      <span key={tag} className="flex items-center gap-2 bg-white border border-slate-100 pl-3 pr-2 py-1.5 rounded-xl text-[11px] font-bold text-slate-600 shadow-sm">
                        #{tag}
                        <button type="button" onClick={() => removeServiceTag(tag)} className="p-0.5 hover:bg-rose-50 text-slate-300 hover:text-rose-500 rounded-md transition-all">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
                    <div className="flex items-center gap-2 w-full">
                      <input 
                        ...
                        className="flex-1 bg-transparent text-xs font-bold outline-none py-1 px-2 border-b border-dashed border-slate-200 focus:border-primary/40 transition-all placeholder:text-slate-300"
                      />
                      <button type="button" onClick={addServiceTag} className="p-2 bg-primary/10 text-primary rounded-xl hover:bg-primary/20 transition-all">
                        <Plus className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
      ...
      <ConfirmationDialog
        ...
        description="Are you sure you want to delete this category? All sub-sectors will be removed."
        confirmText="Delete"
        cancelText="Cancel"
        danger
