            ...
            <GlassCard className={cn(
              ...
              <div className="flex items-center gap-4 sm:gap-5 mb-6 sm:mb-10">
                <div className={cn(
                  ...
                  {plan.name === 'Premium' ? <Zap className="w-6 h-6 sm:w-8 sm:h-8" /> : <Shield className="w-6 h-6 sm:w-8 sm:h-8" />}
                </div>
                <div>
                  <h3 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tighter">{plan.name}</h3>
              ...
              <ul className="space-y-4 sm:space-y-5 mb-8 sm:mb-12">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3 sm:gap-4 text-xs sm:text-sm font-bold text-slate-600">
      ...
      <div className="pt-6 sm:pt-10">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
           ...
           <div className="flex flex-col sm:flex-row gap-4">
              <StatCard title="Subscribers" value="1,402" icon={Users} className="py-3 px-6 h-auto sm:min-w-[200px]" color="primary" />
              <StatCard title="Revenue" value={formatCurrency(12400)} icon={TrendingUp} className="py-3 px-6 h-auto sm:min-w-[200px]" color="blue" />
           </div>
        </div>
        ...
        <GlassCard className="overflow-hidden p-0 border-white/60 shadow-2xl">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 backdrop-blur-md border-b border-slate-100">
                <tr>
                  <th className="px-5 sm:px-8 py-4 sm:py-6 text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Organizer</th>
                  <th className="px-5 sm:px-8 py-4 sm:py-6 text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] hidden sm:table-cell">Plan</th>
                  <th className="px-5 sm:px-8 py-4 sm:py-6 text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] hidden md:table-cell">Auto-renew</th>
                  <th className="px-5 sm:px-8 py-4 sm:py-6 text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] sm:text-right">Expires</th>
                </tr>
              </thead>
                  ...
                  >
                    <td className="px-5 sm:px-8 py-4">
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-slate-100 border-2 border-white shadow-sm overflow-hidden p-0.5">
                           <img src={org.avatar} className="w-full h-full object-cover rounded-[10px]" alt="" />
                        </div>
                        <div>
                           <p className="font-black text-slate-800 text-xs sm:text-sm tracking-tight truncate max-w-[100px] xs:max-w-none">{org.name}</p>
                           <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest sm:hidden">{org.subscriptionPlan}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 sm:px-8 py-4 sm:text-right">
                       <p className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest">{org.subscriptionExpiry}</p>
                       <p className="text-[8px] sm:text-[9px] font-bold text-slate-300 uppercase tracking-widest mt-0.5 sm:hidden">Expires</p>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="p-4 sm:p-6 bg-slate-50/30 border-t border-slate-100 text-center">
             <button className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] hover:text-primary transition-colors outline-none">View all subscribers</button>
          </div>
        </GlassCard>
      </div>
            ...
            <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl border border-slate-200 shadow-2xl p-5 sm:p-10">
              <div className="flex items-center justify-between mb-6 sm:mb-10">
                <div>
                  ...
                  <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Set plan name, price, and features</p>
                </div>
                <button 
                  type="button"
                 ...
                 <div className="space-y-4 sm:space-y-8">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                         <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 font-bold text-sm sm:text-base">$</div>
                         <input 
                          type="number" 
                    ...
                    <div className="space-y-4 p-4 sm:p-8 bg-slate-50 rounded-[1.5rem] sm:rounded-[2.5rem] border border-slate-100 min-h-[180px] sm:min-h-[220px]">
                       <div className="flex flex-wrap gap-2">
                          {planForm.features.map((feat, i) => (
                            <span key={i} className="flex items-center gap-2 bg-white border border-slate-100 pl-3 pr-2 py-2 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-slate-600 shadow-sm">
                       ...
                       <div className="grid grid-cols-1 xs:grid-cols-2 gap-2 mt-4">
                          {['Analytics Access', 'Promotions', 'Visibility Boost', 'Priority Support'].map(f => (
                            <button 
                              key={f} 
                              type="button"
                              onClick={() => !planForm.features.includes(f) && setPlanForm({...planForm, features: [...planForm.features, f]})} 
